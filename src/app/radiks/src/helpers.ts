...
export const clearStorage = () => {
  SecureStorage.removeItem(GROUP_MEMBERSHIPS_STORAGE_KEY);
};

export const userGroupKeys = async () => {
  const keysString = await SecureStorage.getItem(GROUP_MEMBERSHIPS_STORAGE_KEY);
...
export const addPersonalSigningKey = async (signingKey) => {
  const keys = await userGroupKeys();
  keys.personal = {
    _id: signingKey._id,
    ...signingKey.attrs,
  };
  await SecureStorage.setItem(GROUP_MEMBERSHIPS_STORAGE_KEY, JSON.stringify(keys));
};

export const addUserGroupKey = async (userGroup) => {
  const keys = await userGroupKeys();
